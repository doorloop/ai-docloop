...
function removeGlobPattern(path: string): string {
  return path.endsWith(GLOB_PATTERN_SUFFIX)
    ? path.slice(0, -GLOB_PATTERN_SUFFIX.length)
    : path;
}
...
function removeRelativePrefix(path: string): string {
  return path.startsWith(RELATIVE_PATH_PREFIX)
    ? path.slice(RELATIVE_PATH_PREFIX.length)
    : path;
}
...
function normalizePathString(path: string): string {
  // Normalize path separators first so glob pattern detection works correctly
...
function findMatchingScope(
  fileSegments: string[],
  scopes: PathScopeConfig[],
): PathScopeConfig | null {
  const matchingScopes = scopes.filter((scope) =>
    isPathPrefix(scope.scopeRootSegments, fileSegments),
  );
  ...
  return matchingScopes.reduce((prev, current) =>
    current.scopeRootSegments.length > prev.scopeRootSegments.length ? current : prev,
  );
}

export function mapFilesToDocRoots(
  files: string[],
  pathScopes: PathScopeConfig[],
  depthFromScope: number,
): Map<string, string[]> {
